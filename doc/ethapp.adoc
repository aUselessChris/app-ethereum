...
Generates a random 32-bit long value and return it.
...
This command provides a trusted Ethereum domain name (ENS) to be displayed in place of the address it is associated to.
It shall be run just before a transaction involving the associated address that would be displayed on the device.

The signature is computed on :

challenge (BE) || address || domain name || chain ID (BE) || key ID || algorithm ID
